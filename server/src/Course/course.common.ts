...
{ 
  try{
        let discountedPrice=getPriceAfterDiscount(price);

    const outputCurrency= CountryToCurrency.getParamByParam('countryName', country, 'currency');

    let currencyConverter = new CC({from:price.currency, to:outputCurrency, amount:discountedPrice});
    const convertedPrice= await currencyConverter.convert();
    return {...price, currentValue:convertedPrice, currency:outputCurrency};
