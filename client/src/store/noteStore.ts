...
type INoteStore = {
	...
	getNotesByCourseNameAndLessonId: (courseName?:string,lessonId?:string) => INote[] ;
		...
		persist(
			(set, get) => ({
				...
				updateTag: (tag: Partial<ITag>) => {
					...
					const newNotes = [...notes].map(note => {
						if (note.tags) {
							const updatedTags = note.tags.map(t => t.id === tag.id ? { ...t, ...tag } : t);
							return {
								...note,
								tags: updatedTags
							};
				...
				searchNoteById: (id) => {
					...
					const { notes } = get();
					const newNotes = notes?.filter(note => note.id === id);
					return (newNotes && newNotes.length > 0) ? (newNotes[0] as INote) : null;
				},
				...
				getNotesByCourseNameAndLessonId: (courseName?:string,lessonId?:string) => {
					const { notes } = get();
					return notes.filter(note => (!courseName || (note.courseName && note.courseName === courseName)) && (!lessonId || (note.lessonId && note.lessonId === lessonId)));
				}
			}),
			{
