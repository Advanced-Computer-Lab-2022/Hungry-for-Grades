...
export default function ExactFilter(props: {
  x: FilterElement;
  ...
  const toShow2 = props?.x?.values?.map((b: string) => {
    return (
      <div
        key={
          Math.random() +
          1091124142821217 * 2301131482 * -1 * Math.random() -
          Math.random()
        }
      >
        <button
          style={{ fontSize: '0.9rem', fontWeight: '600', color: '#A00407' }}
          type='button'
          onClick={() => {
            setVisible(!visible);
            props?.fun(new Set());
  ...
  return (
    <div
      key={Math.random() + 1289741 * 23214124 * Math.random() - Math.random()}
      style={{
        display: 'inline-block',
        marginBottom: `${rqm}rem`,
        marginRight: '2rem'
      }}
    >
      <div>
        ...
        >
          {props?.x?.actualValue} <AiFillCaretDown />
        </button>
