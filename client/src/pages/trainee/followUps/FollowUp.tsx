...
export default function FollowUp() {
  ...
  let i = -1;
  const img = ((trainee == 'true')? 'https://thebenclark.files.wordpress.com/2014/03/facebook-default-no-profile-pic.jpg?w=640': data?.img);
  ...
  const toShow = report?.followUp?.map((message : Message) => 
  {
    ...
    return (
      ...
      <div key = {++i} className = "row my-2">
        ...
        <div className = "col-1" >
          {!sender &&
          <img alt = {'Some Dude'} height = '35' src = {img} style={{borderRadius:'50px'}} width='40' /> 
          }
        </div> 
        <div className = {`col-11 d-flex ${sender? 'justify-content-end':'justify-content-start'}`}>
          <div className = {` ${sender? styles.sender||'':styles.receiver || ''}`} style={{minWidth:'3rem', padding:'0.5rem 0.7rem'}}>
            {message.content}
          </div>
  ...
  async function SendMessage() {
    ...
    message.URL = `/report/${report?._id as string}/user/${user?._id as string}`;
  ...
  <div className = "container" style={{ maxWidth:'40rem', margin:'3rem auto'}}>
    <div className = 'text-center' style={{fontSize:'1.5rem', fontWeight:'500', color:'#a00407', marginBottom:'2rem'}}>
    ...
    <div className = {styles.send_message} >
      <textarea className={styles.message} value={txt} onChange={(e)=>setTxt(e.target.value )}  />
      <button style={{border:'none'}} type = 'button' onClick={()=>SendMessage()}>
