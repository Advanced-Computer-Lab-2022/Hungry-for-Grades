...
export default function ReviewSection(props : {instructrID : string}) {
  ...
  */
  ...
  const {data, isLoading, isError, error} = useQuery(['person-get-my-instructor-reviewwwww', location, update],
  ...
  async function handleDelete() 
  {
    ...
    rev.URL = `/instructor/rating/${props.instructrID}/trainee/${user?._id as string}`;
  ...
  return (
    ...
    <>
    <div style={{fontWeight:'700', fontSize:'1.5rem'}}>Your Review</div>
    <ReviewContainer comment={review?.comment} country={review?._trainee?.country} createdAt={review?.createdAt.toString()} img={review?._trainee?.profileImage} name={review?._trainee?.name} rating={review?.rating} />
    <button style={{border:'none', backgroundColor:'#a00407', color:'white', borderRadius:'10px'}} type = 'button'
    onClick={()=> setShowForm(!showForm)}>Edit</button>
    <button style={{border:'none', color:'#a00407', borderRadius:'10px'}} type = 'button' onClick = {()=> handleDelete()}>Delete</button>
    </>
   }
   {
   showForm && review != null && review != undefined
   &&
   <AddReview isUpdating instructorID={props?.instructrID} updateFunction={handleUpdate } />
   }
   {
    review && null || review == undefined &&
    <AddReview instructorID={props?.instructrID} isUpdating={false} updateFunction={handleUpdate} />
   }
